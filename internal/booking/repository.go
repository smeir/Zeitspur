...
func (r *Repository) GetDay(ctx context.Context, date string) (*DayStatus, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT work_date, booked, booked_at, updated_at
		FROM day_status WHERE work_date = ?
	`, date)
	var ds DayStatus
	var bookedAt sql.NullString
	var updatedAt string
	err := row.Scan(&ds.WorkDate, &ds.Booked, &bookedAt, &updatedAt)
	if err == sql.ErrNoRows {
		return &DayStatus{WorkDate: date, Booked: false}, nil
	}
	if err != nil {
		return nil, err
	}
...
// SetBooked sets the booked flag for a day.
func (r *Repository) SetBooked(ctx context.Context, date string, booked bool) error {
	now := time.Now().UTC().Format(time.RFC3339Nano)
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO day_status (work_date, booked, booked_at, updated_at)
		VALUES (?, ?, ?, ?)
...
func (r *Repository) ListDaysInRange(ctx context.Context, start, end string) ([]*DayStatus, error) {
	...
	for rows.Next() {
		...
		var bookedAt sql.NullString
		var updatedAt string
		if err := rows.Scan(&ds.WorkDate, &ds.Booked, &bookedAt, &updatedAt); err != nil {
			return nil, err
		}
		if bookedAt.Valid {
			t, _ := time.Parse(time.RFC3339Nano, bookedAt.String)
